...
# ----------------------
# 强制使用黑体（完整中文字体，SimsunExtG 不含常用字）
font_path = Path("C:/Windows/Fonts/simhei.ttf")
if not font_path.exists():
    raise FileNotFoundError("找不到中文字体 simhei.ttf")
...
def run_media_propagation_simulation(frequency, wavelength, amplitude, er1, ur1, er2, ur2,
    ...
    try:
        ...
        ax1.set_title(f'电磁波跨介质传播 f={f / 1e6:.1f}MHz', fontsize=12, fontproperties=font_prop)
        ax1.set_xlabel('传播方向x', fontsize=10, fontproperties=font_prop)
        ax1.set_ylabel('磁场H', fontsize=10, fontproperties=font_prop)
        ax1.set_zlabel('电场E', fontsize=10, fontproperties=font_prop)

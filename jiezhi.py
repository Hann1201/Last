...
def run_media_propagation_simulation(frequency, wavelength, amplitude, er1, ur1, er2, ur2,
    ...
    try:
        ...
        # 电场用左侧Y轴（蓝色）
        ax2_e = ax2
        line_e, = ax2_e.plot(x, E_1d, 'b-', lw=2.5, label='电场 E(x)', alpha=0.8)
        ax2_e.set_xlabel('传播方向 z (m)', fontsize=10, fontproperties=font_prop)
        ax2_e.set_ylabel('电场强度 E (V/m)', fontsize=10, fontproperties=font_prop, color='blue')
        ax2_e.tick_params(axis='y', labelcolor='blue')
        ax2_e.set_ylim(-amplitude * 1.2, amplitude * 1.2)

        # 磁场用右侧Y轴（橙色）
        ax2_h = ax2_e.twinx()
        line_h, = ax2_h.plot(x, H_1d, color='orange', lw=2, label='磁场 H(x)', alpha=0.8)
        ax2_h.set_ylabel('磁场强度 H (A/m)', fontsize=10, fontproperties=font_prop, color='orange')
        ...
        ax2_e.set_title('电场与磁场沿传播方向变化', fontsize=11, fontproperties=font_prop)
